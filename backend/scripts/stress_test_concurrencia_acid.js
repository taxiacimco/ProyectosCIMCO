...
/**
 * Ubicación: backend/scripts/stress_test_concurrencia_acid.js
 * Misión: Validar la prevención de condiciones de carrera (Race Conditions) mediante aislamiento ACID.
 */
...
if (ENTORNO_ACTUAL === 'production') {
    console.error("\n🚨 [CIMCO-SEGURIDAD] Abortando: Ataques concurrentes bloqueados en producción.");
    process.exit(1);
}
...
async function simularAsaltoConcurrenteACID() {
    ...
    const poolPromesas = Array.from({ length: TOTAL_HILOS }, (_, i) => dispararHiloPeticion(i + 1, viajeIdComun));
    ...
    if (exitosos === 1) {
        ...
        console.log("\n⚠️ [ALERTA] Ningún hilo pudo consolidar. Revisa si el conductor ya está ocupado.");
    }
    console.log("==================================================================");
}
