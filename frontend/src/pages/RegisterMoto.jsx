...
const RegisterMoto = () => {
  ...
  const [tipoMoto, setTipoMoto] = useState('mototaxi');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        rol: tipoMoto // 'mototaxi', 'motoparrillero' o 'motocarga'
      };
      ...
      setError(err.response?.data?.message || "Fallo en el registro del dispositivo móvil.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    // 1. Contenedor exterior (Fondo claro e iluminación superior sutil)
    <div className="min-h-screen bg-slate-50 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-teal-50/30 via-slate-50 to-slate-100 flex items-center justify-center p-4 font-sans transition-colors duration-500">
      ...
      // 2. Tarjeta de registro (Efecto cristal claro y bordes suaves)
      <div className="w-full max-w-lg backdrop-blur-md bg-white/90 border border-slate-200/60 p-8 rounded-2xl shadow-xl shadow-slate-200/50">
        ...
        <div className="mb-6 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-teal-500/[0.06] border border-teal-500/20 rounded-full mb-3">
            ...
            <span className="text-[10px] font-mono tracking-[0.15em] text-teal-600 uppercase font-bold">Módulo Operativo Móvil</span>
          </div>
          {/* Título principal unificado con text-slate-800 */}
          <h2 className="text-slate-800 font-black text-2xl tracking-tight">Registro TAXIA CIMCO</h2>
          {/* Subtítulo unificado con text-slate-500 */}
          <p className="text-slate-500 font-mono text-[10px] tracking-wide mt-1 uppercase font-semibold">Registro de Unidades de Dos Ruedas / Carga</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-slate-500 font-mono text-[10px] uppercase tracking-widest font-bold">Tipo de Unidad</label>
              {/* Select adaptado al tema claro con foco en turquesa */}
              <select className="w-full bg-slate-50 border border-slate-200 p-3 rounded-lg text-slate-800 focus:border-teal-500 focus:ring-1 focus:ring-teal-500 focus:bg-white outline-none transition-all text-sm" value={tipoMoto} onChange={(e) => setTipoMoto(e.target.value)}>
                <option value="mototaxi">Mototaxi Urbano</option>
                <option value="motoparrillero">Motoparrillero Autorizado</option>
                <option value="motocarga">Motocarga de Logística</option>
              </select>
            </div>
            <div className="space-y-1.5">
              <label className="text-slate-500 font-mono text-[10px] uppercase tracking-widest font-bold">Placa del Vehículo</label>
              <input type="text" placeholder="Ej. XYZ123" maxLength="6" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-lg text-slate-800 focus:border-teal-500 focus:ring-1 focus:ring-teal-500 focus:bg-white outline-none transition-all text-sm font-mono uppercase" value={placa} onChange={(e) => setPlaca(e.target.value)} required />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input type="email" placeholder="correo@taxiacimco.com" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-lg text-slate-800 focus:border-teal-500 focus:ring-1 focus:ring-teal-500 focus:bg-white outline-none transition-all text-sm font-mono" value={correo} onChange={(e) => setCorreo(e.target.value)} required />
            </div>
          </div>
        ...
        {/* Enlace de retorno centralizado */}
