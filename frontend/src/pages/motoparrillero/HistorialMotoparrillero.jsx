...
const HistorialMotoparrillero = () => {
    ...
    useEffect(() => {
        const fetchHistorial = async () => {
            ...
            try {
                // Uso de FIRESTORE_PATHS adaptado al flujo de servicios completados por el conductor
                const q = query(
                ...
                console.error("❌ [CIMCO-LOG-ERROR] Fallo en la sincronización de historial Parrillero:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="min-h-screen bg-[#0e0e11] font-mono text-zinc-100 p-6 flex flex-col gap-6 selection:bg-cyan-400 selection:text-black">
            ...
            <header className="flex items-center gap-4 bg-zinc-900 border-4 border-black p-4 shadow-[4px_4px_0px_0px_#000] rounded-none">
                ...
                <div>
                    <h1 className="text-xl font-black uppercase tracking-widest text-white leading-none">Historial Parrillero</h1>
                    <p className="text-[10px] text-cyan-400 uppercase tracking-wider font-bold mt-1">Bitácora de servicios completados en red motoparrillero</p>
                </div>
            </header>
